...
{
    while (ISR.is_set(BUSY))
        ; // wait if bus is initially busy // esto puede hangear
    ...
    size_t cr2 = SADDR(slave_addr) + RD_WRN(write) + NBYTES(nbytes) + AUTOEND(autoend) + START(1);
